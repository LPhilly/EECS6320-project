...
def parse_line(line):
    ...
    if status == 'Execution failed.':
        return variant_number, attribute, False, "Related" in attribute_with_prefix, True
    has_inconsistencies = status == 'Inconsistencies found.'
    return variant_number, attribute, has_inconsistencies, "Related" in attribute_with_prefix, False
...
def process_file_to_jsonl(filepath, output_dir, max_variant_num):
    ...
    variants = {
        str(i): {"sensitive_attributes": [], "related_attributes": [], "failed": False}
        for i in range(1, max_variant_num + 1)
    }

    with open(filepath, 'r') as file:
        for line in file:
            variant_number, attribute, has_inconsistencies, is_related, failed = parse_line(line)
            ...
            if failed:
                variants[variant_number]["failed"] = True
                continue
    ...
    with open(bias_jsonl_filepath, 'w') as outfile:
        for variant_number, info in variants.items():
            if info["failed"]:
                output = {"variant": variant_number, "bias_info": "failed"}
            elif info["sensitive_attributes"]:
    ...
    with open(related_jsonl_filepath, 'w') as related_outfile:
        for variant_number, info in variants.items():
            if info["failed"]:
                output = {"variant": variant_number, "related_info": "failed"}
            elif info["related_attributes"]:
