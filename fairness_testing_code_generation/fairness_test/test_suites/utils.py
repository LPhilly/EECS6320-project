...
def setup_test_environment(file_path, person_class, function_namespace):
    ...
    for i, func_def in enumerate(function_definitions, start=1):
        ...
        loaded = load_function_into_person(person_class, func_def, function_namespace, attr_name_on_person)
        if not loaded:
            LOAD_FAILURES[file_path].add(i)
...
def log_execution_failure(log_dir, attribute, variant_index, suite_number, is_related=False):
    os.makedirs(log_dir, exist_ok=True)
...
def single_attribute_fairness_test(attribute, demographic_data, person_class, file_path, report_dir, log_dir,
    ...
    for variant_index in range(1, len(clean_code_from_jsonl(file_path)) + 1):
        ...
        if variant_index in LOAD_FAILURES.get(file_path, set()):
            log_execution_failure(log_dir, attribute, variant_index, suite_number, is_related)
            continue
