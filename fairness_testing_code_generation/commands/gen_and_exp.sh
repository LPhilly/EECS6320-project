if [ "$#" -le 5 ]; then
    echo "Usage: bash gen_and_exp.sh [sampling] [temperature] [prompt_style] [data_path] [model_dir] [model_name] [strategy]"
    exit
fi
...
echo generate_code.py "$DATA_PATH" "$MODEL_DIR"/response "$SAMPLING" "$TEMPERATURE" "$PROMPT_STYLE" "$MODEL_NAME" "$STRATEGY"
...
echo pass_at_attribute.py "$MODEL_DIR" "$SAMPLING"
...
python generate_code.py "$DATA_PATH" "$MODEL_DIR"/response "$SAMPLING" "$TEMPERATURE" "$PROMPT_STYLE" "$MODEL_NAME" "$STRATEGY"
...
python pass_at_attribute.py "$MODEL_DIR" "$SAMPLING"
